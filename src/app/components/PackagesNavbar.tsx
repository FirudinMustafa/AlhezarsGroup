      ...
      <nav className={`w-full max-w-[720px] rounded-2xl backdrop-blur-2xl transition-all duration-300 border ${scrolled ? 'bg-black/75 border-white/[0.07]' : 'bg-black/40 border-white/[0.05]'}`}>
        <div className="px-5">
          <div className="flex items-center justify-between h-[48px]">
                ...
                {pkgOpen && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-48 bg-black/90 backdrop-blur-2xl border border-white/[0.07] rounded-xl overflow-hidden p-1 shadow-lg shadow-black/50">
                    <Link href="/paketler/sosial-media" className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg hover:bg-white/[0.06] text-[13px] ${activePage === 'sosial-media' ? 'text-white font-medium' : 'text-white/45 hover:text-white'}`}>
                      <Instagram className="w-3.5 h-3.5 flex-shrink-0 text-fuchsia-400" /> Sosial Media
                    </Link>
                    <Link href="/paketler/web-dizayn" className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg hover:bg-white/[0.06] text-[13px] ${activePage === 'web-dizayn' ? 'text-white font-medium' : 'text-white/45 hover:text-white'}`}>
                      <Globe className="w-3.5 h-3.5 flex-shrink-0 text-blue-400" /> Web Dizayn
                    </Link>
                  </div>
                )}
              </div>
            ...
            <button onClick={() => setOpen(!open)} className="md:hidden p-1.5 text-white/50 hover:text-white transition-colors">
              {open ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
            </button>
          </div>
        </div>
        {open && (
          <div className="md:hidden overflow-hidden border-t border-white/[0.05]">
            <div className="px-5 pb-4 flex flex-col gap-1">
              <div>
                <button onClick={() => setMobilePkgOpen(!mobilePkgOpen)} className="flex items-center justify-between w-full py-3 text-[15px] text-white/55 hover:text-white transition-colors">
                  Paketlər <ChevronDown className={`w-4 h-4 transition-transform ${mobilePkgOpen ? 'rotate-180' : ''}`} />
                </button>
                {mobilePkgOpen && (
                  <div className="overflow-hidden">
                    <Link href="/paketler/sosial-media" onClick={() => setOpen(false)} className={`block py-2.5 pl-4 text-[15px] ${activePage === 'sosial-media' ? 'text-white font-medium' : 'text-white/40'}`}>Sosial Media</Link>
                    <Link href="/paketler/web-dizayn" onClick={() => setOpen(false)} className={`block py-2.5 pl-4 text-[15px] ${activePage === 'web-dizayn' ? 'text-white font-medium' : 'text-white/40'}`}>Web Dizayn</Link>
                  </div>
                )}
              </div>
              {LINKS.map((l) => (
                <Link key={l.label} href={l.href} onClick={() => setOpen(false)} className="py-3 text-[15px] text-white/55 hover:text-white transition-colors border-b border-white/[0.04] last:border-0">{l.label}</Link>
              ))}
              <Link href="/#elaqe" onClick={() => setOpen(false)} className="mt-2 text-center py-2.5 text-[13px] font-medium bg-[#7c3aed] text-white rounded-full hover:bg-[#6d28d9] transition-colors">Başlayaq</Link>
            </div>
          </div>
        )}
      </nav>
